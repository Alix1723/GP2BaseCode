...
float4 PS(PS_INPUT input):SV_TARGET
{
	...
	float bMap = baseMapTexture.Sample(WrapPointSampler,input.texCoord);
	float lMap = lightMapTexture.Sample(WrapPointSampler,input.texCoord);
	
	return ((ambientMaterial*ambientLightColour*bMap) 
	+(diffuseLightColour*diffuseHighlight*lMap)
	+(specLightColour*specular*lMap)
	);
}
